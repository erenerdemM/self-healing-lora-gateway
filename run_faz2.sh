#!/bin/bash
# =============================================================================
# run_faz2.sh — Auto-generated by generate_massive_topology.py
# =============================================================================
# Toplam config : 84
...
for cfg_idx in $(seq "${FROM_CFG}" "${TO_CFG}"); do
    ...
    if [[ ${JOBS} -gt 1 ]] && command -v parallel &>/dev/null; then
        ...
        # xargs -P fallback (GNU Parallel yok)
        export _RUN_CONFIG="${config}"
        seq 0 $(( RUNS_PER_CONFIG - 1 )) | \
            xargs -P "${JOBS}" -I{} bash -c 'run_one "$_RUN_CONFIG" "$1"' _ {}
    else
